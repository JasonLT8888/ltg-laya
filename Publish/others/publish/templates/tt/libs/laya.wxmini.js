window.wxMiniGame = function (exports, Laya) {
  ...
  class MiniSound extends Laya.EventDispatcher {
    ...
    static _createSound() {
      MiniSound._id++;
      let result = MiniAdpter.window.wx.createInnerAudioContext();
      // console.log("create audio");
      return result;
    }
    load(url, readyUrl) {
      // console.log("loadUrl", url);
      // console.log('_bgMusic', Laya.SoundManager._bgMusic);
      ...
      this.url = url;
      this.readyUrl = readyUrl ? readyUrl : url;
      if (MiniSound._audioCache[this.readyUrl]) {
        this.event(Laya.Event.COMPLETE);
        return;
      }
      if (MiniAdpter.autoCacheFile && MiniFileMgr.getFileInfo(url)) {
        let fileInfo = MiniFileMgr.getFileInfo(url);
        this.onDownLoadCallBack(url, 0, fileInfo.readyUrl);
      }
      else {
    ...
    onDownLoadCallBack(sourceUrl, errorCode, tempFilePath = null) {
      if (!errorCode) {
        ...
        if (MiniSound._audioCache[this.readyUrl] == null) {
    ...
    play(startTime = 0, loops = 0) {
      ...
      if (this.url == Laya.SoundManager._bgMusic) {
        ...
        if (this.readyUrl == null) {
          this.readyUrl = this.url;
        }
        if (MiniSound._audioCache[this.readyUrl]) {
      ...
      if (MiniAdpter.autoCacheFile && MiniFileMgr.getFileInfo(this.url)) {
        ...
        if (fileObj.tempFileName) {
          tSound.src = this.url = fileObj.tempFilePath;
        } else {
          tSound.src = this.url;
        }
      }
  ...
  class MiniLoader extends Laya.EventDispatcher {
    ...
    static onDownLoadCallBack(sourceUrl, thisLoader, errorCode, tempFilePath = null) {
      if (!errorCode) {
        ...
        sound.load(sourceUrl, fileNativeUrl);

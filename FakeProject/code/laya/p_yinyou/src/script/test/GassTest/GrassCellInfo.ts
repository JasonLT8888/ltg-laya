...
export class GrassCellInfo {
    ...
    constructor(maxGrassNums: number, cellSize: number, privotPos: Laya.Vector3) {
        ...
        this.updateGrassPos();
        this.bound = new Laya.BoundBox(
            new Laya.Vector3(this.privotPos.x - this.size / 2,
                this.privotPos.y, this.privotPos.z - this.size / 2),
            new Laya.Vector3(this.privotPos.x + this.size / 2,
                this.privotPos.y + this.grassHight,
                this.privotPos.z + this.size / 2));
    }

window.ttMiniGame = function (exports, Laya) {
	...
	class MiniFileMgr {
		...
		static read(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "") {
			...
			MiniFileMgr.fs.readFile({
				filePath: fileUrl, encoding: encoding, success: function (data) {
				...
				MiniFileMgr.fs.getFileInfo({
					...
					success: function (data) {
						...
						MiniFileMgr.fs.copyFile({
							srcPath: tempFilePath, destPath: saveFilePath, success: function (data2) {
		...
		static deleteFile(tempFileName, readyUrl = "", callBack = null, encoding = "", fileSize = 0) {
			...
			MiniFileMgr.fs.unlink({
				filePath: deleteFileUrl, success: function (data) {
					if (tempFileName != "") {
						var saveFilePath = MiniFileMgr.getFileNativePath(tempFileName);
						MiniFileMgr.fs.copyFile({
							srcPath: tempFileName, destPath: saveFilePath, success: function (data) {
							}, fail: function (data) {
								callBack != null && callBack.runWith([1, data]);
							}
						});
		...
		static writeFilesList(fileurlkey, filesListStr, isAdd) {
			...
			MiniFileMgr.fs.writeFile({
				filePath: listFilesPath, encoding: 'utf8', data: filesListStr, success: function (data) {
				}, fail: function (data) {
				}
			});
	...
	class MiniInput {
		...
		static inputFocus(e) {
			...
			TTMiniAdapter.window.tt.showKeyboard({
				defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: _inputTarget["confirmType"] || 'done', success: function (res) {
				}, fail: function (res) {
				}
			});
		...
		static hideKeyboard() {
			TTMiniAdapter.window.tt.offKeyboardConfirm();
			TTMiniAdapter.window.tt.offKeyboardInput();
			TTMiniAdapter.window.tt.hideKeyboard({
	...
	class MiniLoader extends Laya.EventDispatcher {
		...
		_loadHttpRequestWhat(url, contentType) {
			...
			if (Laya.Loader.preLoadedMap[url])
				...
				var tempurl = Laya.URL.formatURL(url);
				var noCache = !TTMiniAdapter.AutoCacheDownFile;
				if (!MiniFileMgr.isLocalNativeFile(url)
					&& !MiniFileMgr.getFileInfo(tempurl)
					&& url.indexOf(TTMiniAdapter.window.tt.env.USER_DATA_PATH) == -1
					&& (tempurl.indexOf("http://") != -1 || tempurl.indexOf("https://") != -1)
					&& noCache) {
					if (enable_log)
						console.log('---1无缓存', tempurl);
					thisLoader._loadHttpRequest(tempurl, contentType, thisLoader,
						thisLoader.onLoaded, thisLoader, thisLoader.onProgress, thisLoader, thisLoader.onError);
				}
				else {
					var fileObj = MiniFileMgr.getFileInfo(Laya.URL.formatURL(url));
					if (tempurl.endsWith('.json')) {
						fileObj = null;
						if (enable_log)
							console.log('json强制走远程');
					}
					if (fileObj) {
						if (enable_log)
							console.log('---2有缓存', tempurl);
						fileObj.encoding = fileObj.encoding == null ? "utf8" : fileObj.encoding;
						MiniFileMgr.readFile(MiniFileMgr.getFileNativePath(fileObj.md5), encoding,
							new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), url);
					}
					else if (thisLoader.type == "image" || thisLoader.type == "htmlimage") {
						if (enable_log)
							console.log('---3', tempurl);
						thisLoader._transformImgUrl(url, contentType);
					}
					else {
						if (contentType != Laya.Loader.IMAGE
							&& ((tempurl.indexOf("http://") == -1
								&& tempurl.indexOf("https://") == -1) || MiniFileMgr.isLocalNativeFile(url))) {
							if (enable_log)
								console.log('---4有缓存', tempurl);
							MiniFileMgr.readFile(url, encoding,
								new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), url);
						}
						else {
							if (enable_log)
								console.log('---5无缓存', tempurl);
							MiniFileMgr.downFiles(encodeURI(tempurl), encoding,
								new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), tempurl, true);
						}
					}
				}
			}
	...
	TTMiniAdapter.AutoCacheDownFile = true;
